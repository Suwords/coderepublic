...
object lesson06_sql_functions {
  def main(args: Array[String]): Unit = {
    ...
    val res01: DataFrame = ss.sql("select ta.name,ta.class,tb.score + 20 + 80 " +
      "from " +
      "(select name, class from users ) as ta " +
      "join " +
      "(select name, score from uses where score > 10) as tb " +
      "on ta.name = tb.name " +
      "where tb.score > 60")
    res01.show()
    res01.explain(true)
    ...
    val res02: DataFrame = ss.sql("select name, " +
      "sum(score) " +
      "from users " +
      "group by name " +
      "order by name desc")
    ...
    // udf
    ss.udf.register("ooxx1", new MyAggFun) // avg
    ss.sql("select name, " +
      "ooxx1(score) " +
      "from users " +
      "group by name ").show()
